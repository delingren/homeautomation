...
// Represents a contact sensor typically used by security system to monitor the status of
// doors and windows. The sensor is connected to a pullup input pin and the ground. When
// it's open, the pin is pulled up high. When it's closed, it's pulled to the ground.
// Debouncing is done in the software.
struct DEV_DOORWINDOW: Service::ContactSensor {
  ...
  void loop() {
    unsigned long millis_now = millis();
    // millis() overflows after about 50 days. So, if millis_now < millis_last, that's what happened.
    unsigned long millis_lapsed = millis_now >= millis_last 
      ? millis_now - millis_last
      : 0xFFFFFFFF - (millis_last - millis_now) + 1;
    
    
    if (need_to_update && millis_lapsed > millis_debounce) {
      // We only update if 
      //   * An interrupt has occured since the last update, or we have never updated since booting, reflected by need_to_update.
      //   * Enough time has passed since the last interrupt, for debouncing.
      // And we read the pin again here since we could have missed an interrupt or two due to the bouncing.
...
void setup() {
  ...
  homeSpan
    .setStatusPin(26)
    .setControlPin(27)
    .begin(Category::Bridges, "Doors and Windows");
